# -*- coding: utf-8 -*-
# ============================================================
# Painel de RH — STARCHECK (multi-meses via Índice no Google Sheets)
# ============================================================
...
# ------------------ CONFIG ------------------
st.set_page_config(page_title="Painel de RH — StarCheck", layout="wide")
st.title("Painel de RH — StarCheck")
...
.card h4{margin:0 0 6px;font-size:13px;color:#0b4a6f;font-weight:800}
...
# ------------------ LEITOR GENÉRICO (detecta header) ------------------
...
def read_rh_month(file_id: str, ym_from_index: str) -> Tuple[pd.DataFrame, str]:
    ...
    c_superv  = _find_col(cols, "SUPERVISOR", "SUPERVISORA", "GERENTE")
    ...
    out["CIDADE"] = out["CIDADE"].astype(str).apply(_clean_city)
...
def read_training_month(file_id: str, ym_from_index: str) -> pd.DataFrame:
    ...
    c_superv = _find_col(cols, "SUPERVISOR", "SUPERVISORA")  # opcional, se existir
...
# ============================================================
# REGRAS STARCHECK: SUPERVISORAS + GERENTE GERAL
# ============================================================
SUPERVISORA_BY_CITY = {
    "BACABAL": "SAMMYRA JISELE BRITO REIS",
    "CODÓ": "SAMMYRA JISELE BRITO REIS",
    "CODO": "SAMMYRA JISELE BRITO REIS",

    "BALSAS": "GEISE ALINE MACEDO DE MEDEIROS",
    "PINHEIRO": "GEISE ALINE MACEDO DE MEDEIROS",

    "SÃO LUÍS": "CHRISTIANE SILVA GUIMARÃES",
    "SAO LUIS": "CHRISTIANE SILVA GUIMARÃES",
    "CAXIAS": "CHRISTIANE SILVA GUIMARÃES",
}

SUPERVISORA_GERAL = "ELIVANDY CRUZ DA SILVA"
GERENTE_GERAL = "FREDSON RALFF SOARES CAVALCANTE"

def infer_supervisora(cidade: str, supervisor_campo: str) -> str:
    """
    1) se SUPERVISOR/SUPERVISORA vier preenchido na base, usa ele.
    2) se vier vazio, usa mapeamento por cidade.
    3) fallback: SUPERVISORA_GERAL (Elivandy) para não ficar indefinido.
    """
    sup = _upper(supervisor_campo)
    if sup and sup not in {"NAN", "NONE", "NULL"}:
        return sup
    return SUPERVISORA_BY_CITY.get(_upper(cidade), SUPERVISORA_GERAL)

def infer_gerente_geral(_: str) -> str:
...
if idx.empty:
    st.error("A coluna MÊS do índice não está em um formato reconhecido (ex: 01/2026).")
    st.stop()
...
# Supervisoras / gerente geral
df["SUPERVISORA"] = df.apply(lambda r: infer_supervisora(r.get("CIDADE", ""), r.get("SUPERVISOR", "")), axis=1)
df["GERENTE_GERAL"] = df["CIDADE"].apply(infer_gerente_geral)

df_train = pd.concat(all_train, ignore_index=True) if all_train else pd.DataFrame()
if not df_train.empty:
    # se TREINAMENTO não tiver supervisor, usa por cidade + fallback geral
    df_train["SUPERVISORA"] = df_train.apply(lambda r: infer_supervisora(r.get("CIDADE", ""), r.get("SUPERVISOR", "")), axis=1)
    df_train["GERENTE_GERAL"] = df_train["CIDADE"].apply(infer_gerente_geral)
...
# supervisão e gerente geral
supervisoras = sorted(df_m["SUPERVISORA"].dropna().unique().tolist())
gerente_geral = sorted(df_m["GERENTE_GERAL"].dropna().unique().tolist())
...
with f4:
    f_super = st.multiselect("Supervisora", supervisoras, default=supervisoras)
with f5:
    f_ger = st.multiselect("Gerente (geral)", gerente_geral, default=gerente_geral)

view = df_m.copy()
if f_cidade:
    view = view[view["CIDADE"].isin([_clean_city(x) for x in f_cidade])]
if f_funcao:
    view = view[view["FUNCAO_PAD"].isin([_upper(x) for x in f_funcao])]
if f_status:
    view = view[view["STATUS"].isin([_upper(x) for x in f_status])]
if f_super:
    view = view[view["SUPERVISORA"].isin([_upper(x) for x in f_super])]
if f_ger:
    view = view[view["GERENTE_GERAL"].isin([_upper(x) for x in f_ger])]
...
tab_over, tab_people, tab_gest, tab_abs, tab_turn, tab_train, tab_base = st.tabs(
    ["Visão Geral", "Pessoas", "Supervisão/Gerência", "Absenteísmo", "Turnover", "Treinamentos", "Base"]
)
...
# ============================================================
with tab_gest:
    st.markdown("<div class='section'>Estrutura por supervisora (fim do período)</div>", unsafe_allow_html=True)

    gdf = view.copy()
    gdf["ATIVO_FIM"] = gdf.apply(lambda r: 1 if is_active_asof(r, end_d) else 0, axis=1)

    hc_s = gdf.groupby("SUPERVISORA")["ATIVO_FIM"].sum().reset_index(name="HC_FIM").sort_values("HC_FIM", ascending=False)

    adm_g = adm_period.copy()
    dem_g = dem_period.copy()

    adm_g["SUPERVISORA"] = adm_g.apply(lambda r: infer_supervisora(r.get("CIDADE", ""), r.get("SUPERVISOR", "")), axis=1)
    dem_g["SUPERVISORA"] = dem_g.apply(lambda r: infer_supervisora(r.get("CIDADE", ""), r.get("SUPERVISOR", "")), axis=1)

    mov_s = (
        adm_g.groupby("SUPERVISORA")["NOME"].size().reset_index(name="ADM")
        .merge(dem_g.groupby("SUPERVISORA")["NOME"].size().reset_index(name="DEM"), on="SUPERVISORA", how="outer")
        .fillna(0)
    )
    mov_s["ADM"] = mov_s["ADM"].astype(int)
    mov_s["DEM"] = mov_s["DEM"].astype(int)

    falt_s = gdf.groupby("SUPERVISORA")["FALTAS_MES"].sum().reset_index(name="FALTAS")
    abs_s = hc_s.merge(falt_s, on="SUPERVISORA", how="left").fillna(0)
    abs_s["ABS_%"] = abs_s.apply(lambda r: abs_rate(int(r["FALTAS"]), int(r["HC_FIM"]), dias_uteis_mes), axis=1)

    gdf["ATIVO_INI"] = gdf.apply(lambda r: 1 if is_active_asof(r, start_d) else 0, axis=1)
    hc_ini = gdf.groupby("SUPERVISORA")["ATIVO_INI"].sum().reset_index(name="HC_INI")
    hc_fim = gdf.groupby("SUPERVISORA")["ATIVO_FIM"].sum().reset_index(name="HC_FIM")
    turn_s = hc_ini.merge(hc_fim, on="SUPERVISORA", how="outer").fillna(0).merge(mov_s, on="SUPERVISORA", how="left").fillna(0)
    turn_s["TURN_%"] = turn_s.apply(lambda r: turnover_pct(int(r["ADM"]), int(r["DEM"]), int(r["HC_INI"]), int(r["HC_FIM"])), axis=1)

    c1, c2 = st.columns(2)
    with c1:
        st.altair_chart(bar_with_labels(hc_s, "SUPERVISORA", "HC_FIM", height=320, title="Headcount por supervisora (fim)"), use_container_width=True)
    with c2:
        tmp = mov_s.copy()
        if len(tmp):
            tmp_long = tmp.melt(id_vars=["SUPERVISORA"], value_vars=["ADM", "DEM"], var_name="TIPO", value_name="QTD")
            chart = alt.Chart(tmp_long).mark_bar().encode(
                x=alt.X("SUPERVISORA:N", sort="-y", axis=alt.Axis(labelAngle=0, labelLimit=260), title=""),
                y=alt.Y("QTD:Q", title=""),
                color=alt.Color("TIPO:N", legend=alt.Legend(title="")),
                tooltip=["SUPERVISORA", "TIPO", "QTD"],
            ).properties(height=320, title="Movimentações por supervisora (período)")
            st.altair_chart(chart, use_container_width=True)
        else:
    ...
    c3, c4 = st.columns(2)
    with c3:
        abs_plot = abs_s.sort_values("ABS_%", ascending=False).copy()
        abs_plot["ABS_%"] = abs_plot["ABS_%"].fillna(0)
        st.altair_chart(bar_with_labels(abs_plot, "SUPERVISORA", "ABS_%", height=320, title="Absenteísmo por supervisora (%)", y_format=".2f"), use_container_width=True)
    with c4:
        turn_plot = turn_s.sort_values("TURN_%", ascending=False).copy()
        turn_plot["TURN_%"] = turn_plot["TURN_%"].fillna(0)
        st.altair_chart(bar_with_labels(turn_plot, "SUPERVISORA", "TURN_%", height=320, title="Turnover por supervisora (%)", y_format=".2f"), use_container_width=True)

    st.markdown("<div class='section'>Regras fixas</div>", unsafe_allow_html=True)
    st.write("SAMMYRA JISELE BRITO REIS → Bacabal e Codó")
    st.write("GEISE ALINE MACEDO DE MEDEIROS → Balsas e Pinheiro")
    st.write("CHRISTIANE SILVA GUIMARÃES → São Luís e Caxias")
    st.write("ELIVANDY CRUZ DA SILVA → todas as cidades (fallback)")
    st.write("FREDSON RALFF SOARES CAVALCANTE → gerente geral (todas as cidades)")
...
with tab_abs:
    ...
    with c2:
        chart = alt.Chart(top).mark_bar().encode(
            ...
            tooltip=["NOME", "CIDADE", "FUNCAO_PAD", "SUPERVISORA", "FALTAS_MES"],
...
with tab_turn:
    ...
    if len(adm_list):
        adm_list = adm_list[["NOME", "CIDADE", "FUNCAO_PAD", "SUPERVISORA", "ADMISSAO"]].copy()
    ...
    if len(dem_list):
        dem_list = dem_list[["NOME", "CIDADE", "FUNCAO_PAD", "SUPERVISORA", "DEMISSAO", "MOTIVO_DEMISSAO"]].copy()
...
with tab_train:
    ...
    if df_train is None or df_train.empty:
        ...
        if tr.empty:
            ...
            by_s = (
                tr.groupby("SUPERVISORA")
                .agg(CONVOCADOS=("IS_CONVOCADO", "sum"), PRESENTES=("IS_PRESENTE", "sum"))
                .reset_index()
            )
            by_s["PRESENTES"] = np.minimum(by_s["PRESENTES"], by_s["CONVOCADOS"])
            by_s["COB_%"] = by_s.apply(lambda r: 0.0 if r["CONVOCADOS"] == 0 else (r["PRESENTES"] / r["CONVOCADOS"]) * 100.0, axis=1)
            by_s = by_s.sort_values("COB_%", ascending=False)
            ...
            with colB:
                st.altair_chart(
                    bar_with_labels(by_s, "SUPERVISORA", "COB_%", height=320, title="Cobertura de treinamento por supervisora (%)", y_format=".1f"),
                    use_container_width=True,
                )

            st.markdown("<div class='section'>Detalhamento</div>", unsafe_allow_html=True)
            det = tr[["CIDADE", "SUPERVISORA", "NOME", "IS_CONVOCADO", "IS_PRESENTE", "TREINAMENTO"]].copy()
...
with tab_base:
    ...
    cols_show = [
        "YM", "CIDADE", "SUPERVISORA", "GERENTE_GERAL", "NOME", "CPF", "FUNCAO", "FUNCAO_PAD",
    ...
    try:
        ...
        st.download_button(
            ...
            data=xbuf,
            file_name=f"rh_recorte_starcheck_{ym_sel}.xlsx",
